...
\newline
Analicemos por qué nuestro algoritmo devolvió dicha solución. En primer lugar, el árbol generador mínimo ($AGM$) retornado, se encuentra conformado por las primeras cuatro aristas que ingresamos como parámetro de entrada. Ésto se debe a que nuestro algoritmo primero las ordena por su peso y luego toma las menores para generar el $AGM$. En este caso, como todas las aristas tienen el mismo peso, el algoritmo no cambia su orden y genera el árbol con las primeras cuatro. \newline
\newline
...
\newline
Para ello, cambiemos de lugar la cuarta y quinta aristas \newline
...
\newline
Al realizar dicho cambio, el $AGM$ resultante varía. Esto se debe a que, nuevamente, nuestro algoritmo lo genera con las primeras 4 aristas. Sin embargo, esta distribución sigue siendo solución de nuestro problema ya que el costo de estas sigue siendo 4. \newline
Ahora, observemos qué sucede con nuestro nodo $máster$. El nodo que nos da esta solución es 3, que también era una posible solución del $AGM$ anterior. La única diferencia con la solución anterior es que, ahora, nuestro costo de replicación es igual a 1, lo que significa que éste es menor. \newline
\newline
Este es un claro ejemplo de un grafo que se resolvió dos veces de manera óptima pero cuya solución inicial incidió de distinto modo en la solución del segundo ítem, donde la diferencia culminó en el orden de ingreso de los parámetros.
\newline
\newline
En términos generales, esto puede ocurrir para cualquier grafo en el que la solución del $ítem\ a$ (aplicar el algoritmo de Kruskal) no sea única. Esto se debe a que de todas las soluciones óptimás para este ítem van a tener una solución optima para el $ítem\ b$. Al hacer esto, vamos a encontrar un subconjunto de las soluciones posibles del  $ítem\ a$ que van a tener un menor costo de replicación que las demás.
\newline
\newline
Dos posibles soluciones a esto serian:
\begin{itemize}
\item Para todas las soluciones posibles del  $ítem\ a$, calcular el $ítem\ b$ y quedarnos con la de menor costo de replicación.
...
\subsubsection{Pregunta 2}
 ...
 La principal diferencia entre uno y otro radica en que, al primero ($broadcast$), le alcanza con mandar la información una única vez. Esto se debe a que cada servidor se guarda y envía una copia de la misma, mientras que el segundo envía una copia a un nodo por vez, aumentando el costo de envío de un nodo $a$ a uno nodo $b$ a la sumatoria del costo de cada arista del camino. Entonces para poder resolver el problema es necesario contemplar que dependiendo del nodo que elijamos, éste provoca que los pesos de las aristas se vean modificados. En este nuevo contexto, ya no nos sirve encontrar un arbol generador minimo, ya que este descarta aristas que pueden producir que los pesos que tienen las que quedaron pierdan sentido, con lo cual fue necesario un nuevo enfoque, en el cual por cada nodo que tenemos, calculamos los nuevos pesos de las aristas dependiendo la cantidad de mensajes que tengamos que enviar para que el multicast llegue a todos los servidores y luego calculamos los caminos minimos desde el nodo en cuestion hacia los demas. \newline

Para calcular los pesos de las aristas calculamos la distancia entre el nodo en cuestión y los demás nodos, de esta forma obtenemos la cantidad de paquetes que deberíamos enviar y que camino deben recorrer, luego con algunas cuentas matematicas podemos calcular el peso de las aritas multiplicando la cantidad de paquetes por el costo del enlace.

Luego, una posible solución priorizando el costo de envío de datos es: \newline

\begin{algorithm}[H]
	\SetAlgoLined
	\caption{Minimización por Múlticast}
	\KwIn{Grafo $G$}
	\KwOut{Grafo}
	\ \ \textbf{Grafo} Gmin \\
	\ \ \textbf{Entero} pesoMin = 0 \\
	\ \ \textbf{Por cada}(n \in Nodos(G)) \\
				\ \ \ \ \textbf{Grafo Gpesos} = $CalcularPesos(n,G)$ \\
				\ \ \ \ \textbf{Grafo} G' = $Dijktra$(n,Gpesos) \\
...
Donde $pesoTotal$ calcula la sumatoria del peso de todas las aristas, $CalcularPesos(n,G)$ calcula los nuevos pesos basandose en el nodo elegido y la cantidad de paquetes que debe enviar, eso se logra usando $bfs$ y luego recorriendo los nodos nuevamente para calcular los pesos. $Dijktra$\footnote{Cormen, Thomas H., Charles E. Leiserson, and Ronald L. Rivest. p.658} calcula los caminos mínimos de un nodo hacia todos los demás. De esta manera, nos vamos quedando con aquella solución cuya sumatoria de las aristas posee el menor peso. \newline
